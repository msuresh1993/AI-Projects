...
class MultiAgentSearchAgent(Agent):
    ...
    def __init__(self, evalFn = 'scoreEvaluationFunction', depth = '2'):
        ...
        self.minListtemp = [[] for i in range(self.depth)]
        # self.maxList = []
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def MaxAction(self, gameState, depth):
        ...
        return maxScore
        # pass
    def generateStates(self,gameState,ghost,depth):

        if ghost < gameState.getNumAgents():
            actionGhost = gameState.getLegalActions(ghost)
            if actionGhost == []:
                self.minListtemp[depth].append(gameState)
                return
            for action in actionGhost:
                nextSuccessor = gameState.generateSuccessor(ghost, action)
                    ...
                    #     # print "wtf"
                ghostId = ghost + 1
                self.generateStates(nextSuccessor, ghostId,depth)
        else:
            self.minListtemp[depth].append(gameState)
    ...
    def MinAction(self, gameState, ghost, depth):
        if self.depth == depth:
            return self.evaluationFunction(gameState)
        self.minListtemp[depth] = []
        self.minList[depth] = []
        self.generateStates(gameState,1,depth)
        for state in self.minListtemp[depth]:
